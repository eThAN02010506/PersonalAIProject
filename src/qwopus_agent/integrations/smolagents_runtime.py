...
from qwopus_agent.skills import SkillRegistry, SkillRequest, WorkflowSpec
...
def _has_required_spreadsheet_method(
    ...
    *,
    ...
    if method != "lookup":
        return _has_successful_tool_method(steps, tool_name=tool_name, method=method)
    normalized_question = _normalize_lookup_text(user_question)
    for arguments in _successful_tool_arguments(steps, tool_name=tool_name):
        ...
        if _normalize_lookup_text(lookup_value) in normalized_question:
            return True
    return False


def _successful_tool_arguments(
    steps: list[dict[str, Any]],
    *,
    tool_name: str,
) -> list[dict[str, Any]]:
...
def _apply_missing_spreadsheet_fallbacks(
    steps: list[dict[str, Any]],
    *,
    ...
    if (
        ...
        return ""
    lookup_value = _lookup_value_from_question(user_question)
    if not lookup_value:
        return ""
...
def _lookup_value_from_question(user_question: str) -> str:
...
def _spreadsheet_computation_summary(
    steps: list[dict[str, Any]],
    *,
    user_question: str,
    use_chinese: bool,
    ...
    if use_chinese:
        ...
        if "iqr_outliers" in method_observations:
            count = _extract_outlier_count(method_observations["iqr_outliers"])
            if count == 0:
        ...
        if "zscore_outliers" in method_observations:
            count = _extract_outlier_count(method_observations["zscore_outliers"])
            if count == 0:
                lines.append(
...
def _extract_outlier_count(observation: str) -> int | None:
    ...
    for index, line in enumerate(lines):
        if "outlier_count" not in line or not line.startswith("|"):
            continue
        if index + 2 >= len(lines) or not _is_markdown_table_delimiter(lines[index + 1]):
            continue
