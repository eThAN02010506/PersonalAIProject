...
export default function App() {
  ...
  const [maxEvidenceSources, setMaxEvidenceSources] = useState(12);
        ...
        <header className="topbar">
          ...
          {(mode === "chat" || mode === "documents") && (
            <div className="topbar-controls">
                  ...
                  >
                    <label className="detail-select">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setResponseDetail(event.target.value as ResponseDetail)
                        }
                ...
                >
                  <label className="source-relevance">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setMinSourceRelevance(Number(event.target.value) / 100)
                      }
              ...
              {mode === "chat" && (webSearch || browserAccess || localKnowledge) && (
                <HoverHelp
                  className="source-help"
                  title="Source limit"
                  description="Set the maximum distinct sources retrieved and retained for Agent review. Higher values improve broad comparisons but use more context and can slow weaker models."
                >
                  <label className="source-relevance source-limit">
                    <ListFilter size={15} />
                    <span>Limit</span>
                    <input
                      type="range"
                      ...
                      max="20"
                      step="1"
                      value={maxEvidenceSources}
                      onChange={(event) =>
                        setMaxEvidenceSources(Number(event.target.value))
                      }
                      aria-label="Maximum evidence sources"
                    />
                    <output>{maxEvidenceSources}</output>
                  </label>
                </HoverHelp>
              )}
            </div>
